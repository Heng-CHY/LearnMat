...
import learnmat.loader
import learnmat.builder
...
parser.add_argument('--nu', default=0.01, type=float,
                    help='weight of loss function')
parser.add_argument('--rank', default=0, type=int,
                    help='node rank for distributed training')
parser.add_argument('--dist-url', default='tcp://localhost:10008', type=str,  # tcp://224.66.41.62:23456
...
parser.add_argument('--moco-t', default=0.02, type=float,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.aug_plus:
        ...
        augmentation = [
            transforms.RandomResizedCrop(224, scale=(0.2, 1.)),#224
            transforms.RandomApply([
                transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)
            ], p=0.8),
            transforms.RandomGrayscale(p=0.2),
            transforms.RandomApply([learnmat.loader.GaussianBlur([.1, 2.])], p=0.5),
        ...
        augmentation = [
            transforms.RandomResizedCrop(224, scale=(0.2, 1.)),#原先224，这是为了ViT-L/14@336px 336
            transforms.RandomGrayscale(p=0.2),
    ...
    train_dataset = datasets.ImageFolder(
        traindir,
        learnmat.loader.TwoCropsTransform(transforms.Compose(augmentation)))
    ...
    if args.evaluate:
        validate(val_loader, model, criterion, args, args.start_epoch)
        return

    indx = torch.arange(256)
    indx_train = indx
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        backbone = train(train_loader, model, criterion, optimizer, epoch, args, indx_train)
        # evaluate on validation set
        top1, top5, mAP = validate(val_loader, model, criterion, args, epoch)
        ...
        filtered_dict = {k: v for k, v in model_dict.items() if 'clip' not in k}
      
        filtered_dict = OrderedDict(filtered_dict)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, is_best = is_best, filename=os.path.join('/data3/lsh/checkpoint', 'checkpoint_{:04d}_{}_best_top1_{:.4f}.pth.tar'.format(epoch, dataset_name, best_top1)))


def train(train_loader, model, criterion, optimizer, epoch, args, indx):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        output, target,  backbone,  att_max, gradcam , L_ukd = model(im_q=images[0], im_k=images[1], epoch=epoch, iter=i, indx=indx)

        loss = criterion(output, target)
        loss_cam = F.kl_div(att_max.softmax(dim=-1).log(), gradcam.softmax(dim=-1),
                            reduction='sum')

        total_loss = loss + args.nu * loss_cam + 1.2 * L_ukd #nu =0.01   原来(loss_feat*0.001->没有经过mean(dim=1)操作)
...
def validate(val_loader, model, criterion, args, epoch):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            # compute output
            _, _, featcov16, output = model.module.inference(images)
